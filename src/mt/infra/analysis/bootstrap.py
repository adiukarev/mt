...
from mt.infra.artifact.logs.bootstrap import log_bootstrap_ci, log_bootstrap_ci_fast
	...
	for idx, model_a in enumerate(models):
		for model_b in models[idx + 1:]:
			...
			for _ in range(n_bootstrap):
				...
				metrics_a = calculate_metrics(sampled[sampled["model_name"] == model_a])
				metrics_b = calculate_metrics(sampled[sampled["model_name"] == model_b])
